...
const UsefulWeb = () => {
  ...
  const onOpenModal2 = (priority, title, title_in_marathi, link, _id) => {
  ...
  const deleterecord = async (id) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/delete-website?Id=${id}`,
        {
          method: "DELETE",
          headers: headers,
        }
  ...
  const submithandler = async (e) => {
    if (!title || !title_in_marathi || !link) {
      notifyvalid();
    ...
    const bodydata = {
      ...
      titleInMarathi: DOMPurify.sanitize(title_in_marathi),
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/add-website`,
        {
          method: "POST",
  ...
  const updatehandler = async (e) => {
    if (!titleF || !title_in_marathiF || !linkF) {
      notifyvalid();
    ...
    const bodydata = {
      ...
      titleInMarathi: DOMPurify.sanitize(title_in_marathiF),
      link: DOMPurify.sanitize(linkF),
    };
    if (priorityF) {
      bodydata.priority = priorityF;
    }

    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/update-website?Id=${idF}`,
        {
          method: "PATCH",
          body: JSON.stringify(bodydata),
          headers: headers,
                ...
                <div className="overflow-x-auto max-w-full">
                  <table className="w-full min-w-[800px] mx-auto mt-5 text-left">
                    <tr className="border flex flex-row justify-between bg-gray-100">
                      ...
                      <th className="p-2 w-1/12 border-r">Priority</th>
                      <th className="p-2 w-4/12 border-r">Title</th>
                      <th className="p-2 w-4/12 border-r">Title in Marathi</th>
                      <th className="p-2 w-1/12 border-r">Link</th>
                    </tr>

                    {records.map((record) => {
                      const { priority, title, titleInMarathi, link, _id } =
                        record;
                      return (
                        <tr
                        ...
                        >
                          ...
                          <td className="p-2 w-1/12 border-r">{priority}</td>
                          <td className="p-2 w-4/12 border-r">{title}</td>
