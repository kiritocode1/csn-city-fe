...
const LostFound = () => {
  ...
  const fetchLostFoundStatistics = async () => {
    ...
    try {
      ...
      if (data) {
        setLostFoundStatistics(data.statistics.lostFound.total);
        console.log("data.statistics.lostFound.total", data.statistics.lostFound.total);
      }
    } catch (error) {
  ...
  const fetchrecordsNew = async (page) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/get-lost-found-list?limit=${limit}&page=${page}`,
        {
          method: "GET",
          headers: headers,
        }
  ...
  const deleterecord = async (id) => {
    ...
    try {
      ...
      if (data.success === true) {
        notifydelete();
        fetchrecordsNew(1);
        fetchLostFoundStatistics(); // Refresh statistics after deletion
      } else {
        notifyWarn();
      }
    } catch (error) {
