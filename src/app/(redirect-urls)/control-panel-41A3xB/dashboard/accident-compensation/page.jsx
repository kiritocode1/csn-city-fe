...
const Accident = () => {
	...
	const fetchrecords = async () => {
		try {
			const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/get-accident-compensation`, {
				method: "GET",
			});
	...
	const fetchYears = async () => {
		try {
			const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/get-year`, {
				method: "GET",
			});
	...
	const deleterecord = async (id) => {
		...
		try {
			const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/delete-accident-compensation?Id=${id}`, {
				method: "DELETE",
				headers: headers,
			});
	...
	const submithandler = async (e) => {
		...
		try {
			const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/add-accident-compensation`, {
				method: "POST",
				body: formData,
				headers: headers,
			});
	...
	const updateHandler = async (e) => {
		...
		try {
			const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/update-accident-compensation?Id=${idF}`, {
				method: "PATCH",
				body: formData,
				headers: headers,
			});
					...
					<div className="flex flex-row">
						...
						<div className="w-full lg:w-4/5 mt-20 mx-5 bg-white shadow p-5">
							...
							{isform && (
								<div className="mt-5">
									...
									<div className="flex flex-col mx-3 my-3 space-y-1 mb-5">
										...
										/>
										{f1Err && <p className="text-red-600 text-sm">File size exceeds the limit (5MB). Please choose a smaller file</p>}
									</div>
									...
									<div className="flex flex-col mx-3 my-3 space-y-1 mb-5">
										...
										/>
										{f2Err && <p className="text-red-600 text-sm">File size exceeds the limit (5MB). Please choose a smaller file</p>}
									</div>
									<div>
										<button
							...
							<div className="mb-52 w-full bg-white overflow-x-auto">
								<h1 className="font-semibold text-xl mt-10">Accident Compensation Records</h1>
								...
								<div className="overflow-x-auto max-w-full">
									<table className="w-full min-w-[800px] overflow-x-auto mx-auto mt-5 text-left">
										...
										{records.map((record) => {
											const { cr_no, year, date, psId, comm_aa, fir_file, _id } = record;
											return (
												<tr
												...
												>
													...
													<td className="p-1 w-1/12 border-r">
														<button
															...
															//   onClick={() => deleterecord(_id)}
															onClick={() => onOpenModal2(cr_no, year, date, psId._id, comm_aa, fir_file, _id)}
														>
															<AiFillEdit />
														</button>
													</td>
								...
								>
									<div className="pt-10 px-5 w-[700px]">
										...
										<div className="flex flex-col mx-3 my-3 space-y-1 mb-5">
											...
											/>
											{f1ErrF && <p className="text-red-600 text-sm">File size exceeds the limit (5MB). Please choose a smaller file</p>}
										</div>
										...
										<div className="flex flex-col mx-3 my-3 space-y-1 mb-5">
											...
											/>
											{f2ErrF && <p className="text-red-600 text-sm">File size exceeds the limit (5MB). Please choose a smaller file or Please select a .pdf file</p>}
										</div>
