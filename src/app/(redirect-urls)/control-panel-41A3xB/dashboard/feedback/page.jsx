...
const Feedback = () => {
  ...
  const fetchFeedbackStatistics = async () => {
    ...
    try {
      ...
      if (data) {
        setFeedbackStatistics(data.statistics.feedback.total);
        console.log("data.statistics.feedback.total", data.statistics.feedback.total);
      }
    } catch (error) {
  ...
  const fetchrecordsNew = async (page) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/get-feedback-list?limit=${limit}&page=${page}`,
        {
          method: "GET",
          headers: headers,
        }
  ...
  const deleterecord = async (id) => {
    ...
    try {
      ...
      if (data.success === true) {
        notifydelete();
        fetchrecordsNew(1);
        fetchFeedbackStatistics(); // Refresh statistics after deletion
      } else {
        notifyWarn();
      }
    } catch (error) {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-2 w-1/12 border-r font-bold text-center">
                              {feedbackStatistics - (index)}
                            </td>
                          <td className="p-2 w-2/12 border-r">{fullName}</td>
                          <td className="p-2 w-2/12 border-r">
                            {psId ? psId.name : "N/A"}{" "}
                            {/* Show psId.name if psId exists, else show "N/A" */}
                          </td>
