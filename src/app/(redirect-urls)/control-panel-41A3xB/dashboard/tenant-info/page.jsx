...
const Tenant = () => {
  ...
  const [tenantStatistics, setTenantStatistics] = useState(0);
  ...
  // Fetch statistics
  const fetchTenantStatistics = async () => {
    ...
    try {
      ...
      if (data) {
        setTenantStatistics(data.statistics.tenantInfo.total);
        console.log("data.statistics.tenantInfo.total", data.statistics.tenantInfo.total);
      }
    } catch (error) {
  ...
  const fetchrecordsNew = async (page) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/get-tenant-list?limit=${limit}&page=${page}`,
        {
          method: "GET",
          headers: headers,
        }
  ...
  const deleterecord = async (id) => {
    ...
    try {
      ...
      if (data.success === true) {
        notifydelete();
        fetchrecordsNew(1);
        fetchTenantStatistics(); // Refresh statistics after deletion
      } else {
        notifyWarn();
      }
    } catch (error) {
