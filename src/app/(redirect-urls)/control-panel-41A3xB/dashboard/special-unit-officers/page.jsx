...
const Officer = () => {
  ...
  const [unitIdF, setunitIdF] = useState(null);
  ...
  const onOpenModal2 = (
    ...
    setunitIdF(DOMPurify.sanitize(unitId));
    ...
    setPriorityF(DOMPurify.sanitize(priority));
  ...
  const fetchrecords = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/unit-officers-admin`,
        {
          method: "GET",
        }
      );
  ...
  const deleterecord = async (id) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/delete-unit-officer?Id=${id}`,
        {
          method: "DELETE",
          headers: headers,
        }
  ...
  const submithandler = async (e) => {
    if (!name || !name_in_marathi || !unitId) {
      notifyvalid();
      return;
    }
    ...
    formData.append("unitId", DOMPurify.sanitize(unitId));
    ...
    if (priority) {
      formData.append("priority", DOMPurify.sanitize(priority));
    }
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/add-unit-officer`,
        {
          method: "POST",
          body: formData,
          headers: headers,
  ...
  const updateHandler = async (e) => {
    if (!nameF || !name_in_marathiF || !unitIdF) {
      notifyvalid();
    ...
    formData.append("unitId", DOMPurify.sanitize(unitIdF));
    ...
    if (priorityF) {
      formData.append("priority", DOMPurify.sanitize(priorityF));
    }
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/update-unit-officer?Id=${idF}`,
        {
          method: "PATCH",
          body: formData,
          headers: headers,
              ...
              {isform && (
                <div className="mt-5">
                  <div className="flex flex-col mx-3 my-1 space-y-1 mb-5">
                    ...
                    >
                      <option value="">Select Special Unit</option>
                      {units.map((record) => {
                  ...
                  <div className="flex flex-col mx-3 my-1 space-y-1 mb-5">
                    ...
                    <input
                      ...
                      id="title"
                      name="title"
                      className="text-base border-b border-b-black px-3 py-2  outline-none focus:border-purple-700 w-52"
                      placeholder=""
                      value={priority}
                  ...
                  <div className="pt-10 px-5 w-[700px]">
                    <div className="flex flex-col mx-3 my-1 space-y-1 mb-5">
                      ...
                      <select
                        ...
                        name="location"
                        value={unitIdF}
                        onChange={(e) => setunitIdF(e.target.value)}
                        className="text-base border-b border-b-black px-3 py-2  outline-none focus:border-purple-700 w-full"
                      >
                        <option value="">Select Special Unit</option>
                        {units.map((record) => {
