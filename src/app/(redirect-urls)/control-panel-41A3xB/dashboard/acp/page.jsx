...
const SeniorOfficer = () => {
	...
	const onOpenModal2 = (sr_no, name, name_in_marathi, designation, designation_in_marathi, phone, email, photo, mobile, _id) => {
		setSr_noF(sr_no);
		setNameF(name);
	...
	const fetchrecords = async () => {
		try {
			const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/get-acp`, {
				method: "GET",
			});
	...
	const deleterecord = async (id) => {
		...
		try {
			const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/delete-acp?Id=${id}`, {
				method: "DELETE",
				headers: headers,
			});
	...
	const submithandler = async (e) => {
		...
		if (phone) {
			formData.append("phone", DOMPurify.sanitize(phone));
		}
		...
		try {
			const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/add-acp`, {
				method: "POST",
				body: formData,
				headers: headers,
			});
	...
	const updateHandler = async (e) => {
		...
		if (phoneF) {
			formData.append("phone", DOMPurify.sanitize(phoneF));
		}
		...
		try {
			const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/update-acp?Id=${idF}`, {
				method: "PATCH",
				body: formData,
				headers: headers,
			});
								...
								<div className="mt-5">
									...
									<div className="flex flex-col mx-3 my-1 space-y-1 mb-5">
										...
										<input
											...
											onChange={(e) => setDesignation_in_marathi(e.target.value)}
										/>
									</div>
									...
									<div className="flex flex-col mx-3 my-3 space-y-1 mb-5">
										...
										/>
										{fileErr && <p className="text-red-600 text-sm">File size exceeds the limit (5MB). Please choose a smaller file</p>}
									</div>
								...
								<div className="overflow-x-auto max-w-full">
									<table className="w-full min-w-[800px] mx-auto mt-5 text-left">
										<tr className="border flex flex-row justify-between bg-gray-100">
											...
											<th className="p-2 w-5/12 border-r">Officer Name in Marathi</th>
										</tr>

										{records.map((record) => {
											const { sr_no, name, name_in_marathi, designation, designation_in_marathi, phone, email, photo, mobile, _id } = record;
											return (
												<tr
												...
												>
													...
													<td className="p-1 w-1/12 border-r">
														<button
															...
															//   onClick={() => deleterecord(_id)}
															onClick={() => onOpenModal2(sr_no, name, name_in_marathi, designation, designation_in_marathi, phone, email, photo, mobile, _id)}
														>
															<AiFillEdit />
														</button>
													</td>
													...
													<td className="p-1 w-5/12 border-r">{name_in_marathi}</td>
												</tr>
											);
										})}
									</table>
									...
									<div className="pt-10 px-5 w-[700px]">
										...
										<div className="flex flex-col mx-3 my-1 space-y-1 mb-5">
											...
											<input
												...
												onChange={(e) => setDesignation_in_marathiF(e.target.value)}
											/>
										</div>
										...
										<div className="flex flex-col mx-3 my-1 space-y-1 mb-5">
											...
											/>
											{fileErrF && <p className="text-red-600 text-sm">File size exceeds the limit (5MB). Please choose a smaller file</p>}
										</div>
