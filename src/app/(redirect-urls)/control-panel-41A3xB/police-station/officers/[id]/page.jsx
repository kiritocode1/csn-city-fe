...
const Officer = () => {
  ...
  const [officerStatistics, setOfficerStatistics] = useState(0);
  ...
  // Fetch statistics
  const fetchOfficerStatistics = async () => {
    ...
    try {
      ...
      if (data) {
        // For officers, we'll use a simple count since there's no specific statistics endpoint
        // You might need to create a separate endpoint for officer statistics
        setOfficerStatistics(data.statistics?.officers?.total || 0);
      }
    } catch (error) {
